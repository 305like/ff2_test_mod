...
#include "freak_fortress_2/weapon.sp"
...
public void OnPluginStart()
{
	...
	g_hVersionHud = CreateHudSynchronizer();
	CreateTimer(1.0, Timer_VersionHud, _, TIMER_REPEAT);
	CreateTimer(1.0, WeaponSpecial_OverChargeTimer, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse)
{
	...
	Action weaponResult = WeaponSpecial_PlayerRunCmd(client, buttons, impulse);
	if(weaponResult != Plugin_Continue)
		return weaponResult;
...
public Action Timer_VersionHud(Handle timer)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		...
		{
			SetHudTextParams(0.88, 0.48, 1.1, 200, 200, 200, 80, 0, 0.0, 0.0, 0.0);
			ShowSyncHudText(i, g_hVersionHud, "v%s", CUSTOM_BUILD_VERSION);
		}
	}
	return Plugin_Continue;
}
