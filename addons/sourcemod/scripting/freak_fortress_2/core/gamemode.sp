...
{
	if(Enabled && TF2Tools_Loaded() && SDK_WaitingForPlayers() && FindEntityByClassname(-1, "tf_gamerules") != -1 && GameRules_GetProp("m_bInWaitingForPlayers", 1))
	{
		...
		else if(!GameRules_GetProp("m_bInWaitingForPlayers", 1))
		{
			...
			if(bosses > 0)	// Boss vs Boss
			{
				...
				if(count)
				{
					...
					int maxTeams = TFTeam_Red + Configs_TeamCount();
					int team = TFTeam_Red + (GetTime() % Configs_TeamCount());
					for(int i; i < count; i++)
					{
					...
					for(team = maxTeams - 1; team >= TFTeam_Red; team--)
					{
						count = Preference_GetBossQueue(players, MaxClients, false, team);
						for(int i; i < bosses && i < count; i++)
						{
							if(!Client(players[i]).IsBoss)
							{
								Bosses_CreateFromSpecial(players[i], Preference_PickBoss(players[i], team), team);
								Client(players[i]).Queue = 0;
							}
						}
					}
				}
...
{
	if(Enabled && SDK_WaitingForPlayers() && GameRules_GetProp("m_bInWaitingForPlayers", 1))	// Yes, m_bInWaitingForPlayers is needed here
	{
		Waiting = false;
		Cvar[Tournament].BoolValue = false;
		Cvar[MovementFreeze].BoolValue = false;
		CreateTimer(1.0, Gamemode_TimerRespawn, _, TIMER_FLAG_NO_MAPCHANGE|TIMER_REPEAT);

		delete BackupTimer;
		BackupTimer = CreateTimer(Cvar[WaitingTime].FloatValue + 5.0, Gamemode_BackupWaiting);

		if(SDK_FireEntityOutput() && GameRules_GetProp("m_nGameType") == 4)
		{
...
{
	if(Enabled && SDK_WaitingForPlayers())
...
static Action Gamemode_IntroTimer(Handle timer)
{
	for(int client = 1; client <= MaxClients; client++)
	{
		...
		{
			if(!Client(client).IsBoss || !ForwardOld_OnMusicPerBoss(client) || (!Bosses_PlaySoundToClient(client, client, "sound_intro", .volume = SNDVOL_BOSS) && !Bosses_PlaySoundToClient(client, client, "sound_begin", .volume = SNDVOL_BOSS)))
			{
				...
				if(i == MaxClients)
				{
					...
					if(boss != -1)
					{
						if(!Bosses_PlaySoundToClient(boss, client, "sound_intro", .volume = SNDVOL_BOSS))
							Bosses_PlaySoundToClient(boss, client, "sound_begin", .volume = SNDVOL_BOSS);
					}
				}
			}
		}
...
void Gamemode_RoundStart()
{
	...
	if(Enabled && !GameRules_GetProp("m_bInWaitingForPlayers", 1))
	{
		...
		for(int client = 1; client <= MaxClients; client++)
		{
			...
			{
				int team = GetClientTeam(client);
				if((specTeam || team > TFTeam_Spectator) && teams.FindValue(team) == -1)
					teams.Push(team);
		...
		for(int i; i < bosses; i++)
		{
			...
			for(int client = 1; client <= MaxClients; client++)
			{
				...
				Bosses_GetBossNameCfg(Client(boss[i]).Cfg, buffer, sizeof(buffer), GetClientLanguage(client));
				...
				FPrintToChatEx(client, boss[i], buffer);

				// HUD display
				char bossName[64];
				Bosses_GetBossNameCfg(Client(boss[i]).Cfg, bossName, sizeof(bossName), GetClientLanguage(client));
				char hudMsg[256];
				if(maxlives > 1)
					Format(hudMsg, sizeof(hudMsg), "%t\nHP: %d (x%d)", "Boss Spawn Hud", boss[i], bossName, maxhealth, maxlives);
				else
					Format(hudMsg, sizeof(hudMsg), "%t\nHP: %d", "Boss Spawn Hud", boss[i], bossName, maxhealth);

				SetHudTextParams(-1.0, 0.35, 5.0, 255, 255, 255, 255, 0, 0.0, 0.0, 0.1);
				ShowSyncHudText(client, TeamSyncHud[0], hudMsg);
			}
		}
...
void Gamemode_RoundEnd(int winteam)
{
	...
	*/
	...
	if(total)
	{
		...
		for(int i; i < total; i++)
		{
			if(clients[i] != globalBoss && Client(clients[i]).IsBoss)
			{
				if(winner == teams[i])
				{
					...
					if(Bosses_PlaySoundToClient(clients[i], clients[i], "sound_win", .volume = SNDVOL_BOSS))
						continue;
				}
				else if(globalTeam != winner)
				{
					...
					{
						if(Bosses_PlaySoundToClient(clients[i], clients[i], "sound_fail", .volume = SNDVOL_BOSS))
							continue;
					}
				}
			}
		...
		// Play global sound
		if(globalBoss)
			Bosses_PlaySound(globalBoss, globalSound, globalCount, globalTeam == winner ? "sound_win" : "sound_fail", .volume = SNDVOL_BOSS);
	}
	...
	if(!Enabled || RoundStatus == 1)
	{
		...
		if(setting)
		{
			...
			if(setting > 1 && maxcombined)
			{
				...
				{
					int color[4];
					TF2Tools_GetTeamColor4(team, color);

					float x = (team % 2) ? 0.43 : 0.53;
					float y = 0.12 + ((team / 2) * 0.05);
					for(int i; i < total; i++)
					{
...
void Gamemode_PlayerRunCmd(int client, int buttons)
{
	if(IsPlayerAlive(client))
	{
		if(Enabled && RoundStatus == 1 && !Client(client).IsBoss && !Client(client).MinionType && !GameRules_GetProp("m_bInWaitingForPlayers", 1))
		{
			int team = GetClientTeam(client);
	...
	if(Enabled && RoundStatus == 1 && !Client(client).IsBoss && !Client(client).NoHud && !Client(client).NoDmgHud && !(buttons & IN_SCORE))
	{
		if(Client(client).SapperCooldownFor < time && Client(client).RefreshAt < time)
		{
			...
			char shieldHud[64];
			bool hasShield = WeaponSpecial_GetOverChargeHud(target, shieldHud, sizeof(shieldHud));
			if(target == client)
			{
				if(hasShield)
					ShowSyncHudText(client, PlayerHud, "%t\n%s", "Current Stats Hud", Client(client).TotalDamage, Client(client).Healing, Client(client).TotalAssist, shieldHud);
				else
			...
			{
				if(hasShield)
					ShowSyncHudText(client, PlayerHud, "%t\n%s", "Viewing Stats Hud", target, Client(target).TotalDamage, Client(target).Healing, Client(target).TotalAssist, shieldHud);
				else
