	...
	if(FileExists(filepath))
	{
		...
		if(charset != -1)
		{
			if(ForwardOld_OnLoadCharacterSet(charset, name))
			{
				...
				{
					snap.GetKey(i, packname, sizeof(packname));
					...
					cfg.Get("name", packname, sizeof(packname));
		...
		for(int i; i < entries; i++)	// Boss Packs
		{
			...
			if(entriesBosses)
			{
				...
				if(precache)
				{
					for(int a; a < entriesBosses; a++)
					{
						choosen.Push(a);
					}

					int limit;
					if(cfgPack.GetInt("limit", limit) && limit >= 0)
					{
						// Limit amount of active bosses
						choosen.Sort(Sort_Random, Sort_Integer);
						for(int a = entriesBosses - 1; a >= limit; a--)
						{
							choosen.Erase(a);
						}
					}
				...
				for(int a; a < entriesBosses; a++)	// Bosses in this Pack
				{
			...
			if(!cfgPack.ContainsKey("name"))
				cfgPack.Set("name", key);
			
			pack++;
			
			if(precache && Enabled)
			{
				cfgPack.Get("name", name, sizeof(name));
				SteamWorks_SetGameTitle(entries > 1 ? name : NULL_STRING);
			}
...
ConfigMap Bosses_GetCharset(int charset)
{
	if(!PackList || charset<0 || charset>=PackList.Length)
		return null;
	
	return PackList.Get(charset);
}

int Bosses_GetCharsetName(int charset, char[] buffer, int length, int lang = -1, const char[] string = "name")
{
	if(!PackList || charset<0 || charset>=PackList.Length)
		return 0;
	
	ConfigMap cfg = PackList.Get(charset);
...
static void EquipBoss(int client, bool weapons)
{
	...
	{
		case 0:
			Attrib_Set(client, "healing received penalty", 734, 0.0);
								
		case 1:
			Attrib_Set(client, "reduced_healing_from_medics", 740, 0.0);
	}
	...
	if(creator && GetClientAuthId(client, AuthId_SteamID64, buffer1, sizeof(buffer1)))
	{
		...
		for(i = 0; i < entries; i++)
		{
			...
			{
				Attrib_Set(client, "attach particle effect static", 370, 4.0);
				break;
			}
		}
...
void Bosses_UpdateHealth(int client)
{
	...
	if(maxhealth > 0)
	{
		...
		Attrib_Set(client, "max health additive bonus", 26, float(maxhealth-defaul));
	}
	else
	{
		Attrib_Set(client, "max health additive bonus", 26, 0.0);
	}
}
...
void Bosses_SetSpeed(int client)
{
	...
	if(maxspeed > 0.0)
	{
		...
		Attrib_Set(client, "major move speed bonus", 442, speed / defaul);
		SDKCall_SetSpeed(client);
	}
	else
	{
		Attrib_Set(client, "major move speed bonus", 442, 1.0);
	}
}
...
void Bosses_Remove(int client)
{
	...
	if(Client(client).IsBoss)
	{
		...
		Attrib_Remove(client, "attach particle effect static", 370);
	...
	if((!Enabled || RoundStatus == 1) && IsPlayerAlive(client))
	{
		...
		if(Client(client).IsBoss)
		{
			if(Client(client).PassiveAt <= time)
			{
				...
				// Auto rage gain (~0.1% per second, 0.02 per 0.2s tick)
				// Multiplier based on enemy alive: 3=x2, 2=x4, 1=x8
				float autoRageDmg = Client(client).RageDamage;
				if(autoRageDmg > 0.0 && autoRageDmg < 99999.0)
				{
					float autoRage = Client(client).GetCharge(0);
					float autoMax = Client(client).RageMax;
					if(autoRage < autoMax)
					{
						float gain = 0.02;
						int enemyAlive = TotalPlayersAliveEnemy(GetClientTeam(client));
						if(enemyAlive <= 1)
							gain *= 8.0;
						...
						if(autoRage > autoMax)
							autoRage = autoMax;
						Client(client).SetCharge(0, autoRage);
					}
				}
			}
			...
			if(!Client(client).NoHud && !(buttons & IN_SCORE))
			{
				...
				if(Client(client).RefreshAt < time)
				{
					...
					if(ragedamage >= 0.0 && ragedamage < 99999.0)
					{
						...
						int currentDmg = RoundToFloor(rage * ragedamage / 100.0);
						int maxDmg = RoundToFloor(ragedamage);
						if(rage >= ragemin)
						{
							SetHudTextParams(-1.0, 0.78, 0.35, 255, 64, 64, 255, _, _, 0.01, 0.5);
							Format(buffer, sizeof(buffer), "%s\n%t", buffer, "Boss Rage Ready", RoundToFloor(rage), currentDmg, maxDmg);
						}
						else
						{
							SetHudTextParams(-1.0, 0.78, 0.35, 255, 255, 255, 255, _, _, 0.01, 0.5);
							Format(buffer, sizeof(buffer), "%s\n%t", buffer, "Boss Rage Charge", RoundToFloor(rage), currentDmg, maxDmg);
						}
