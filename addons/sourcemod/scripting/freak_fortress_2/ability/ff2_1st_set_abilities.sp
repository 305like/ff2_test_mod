...
void Rage_Bow(int boss)
{
	...
	int weapon=SpawnWeapon(client, "tf_weapon_compound_bow", 1005, 100, 5, "2 ; 2.5 ; 6 ; 0.5 ; 37 ; 0.0 ; 280 ; 19 ; 137 ; 2.0");
...
public Action Timer_Rage_Explosive_Dance(Handle timer, int boss)
{
	...
	if(count<=35 && IsPlayerAlive(client))
	{
		...
		for(int i; i<5; i++)
		{
			...
			DispatchKeyValueFloat(explosion, "DamageForce", 20.0);

			SetEntProp(explosion, Prop_Data, "m_iMagnitude", 100, 4);
