...
#define PLUGIN_NAME "ff2_sarysapub1"
...
public Plugin:myinfo = {
	...
	author = "sarysa, Forked by Nopied",
	version = "20190602",
}
...
static const char g_strPropName[][] = {
	"jump pad",
	"angle jump pad",
	"slicer",
	"platform"
...
public Action:Timer_PostRoundStartInits(Handle:timer)
{
	...
	for (new clientIdx = 1; clientIdx < MAX_PLAYERS; clientIdx++)
	{
		...
		if (FF2_HasAbility(bossIdx, PLUGIN_NAME, RFM_STRING))
		{
			PluginActiveThisRound = true;
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RFM_STRING, "classname", RFM_WeaponName[clientIdx], MAX_WEAPON_NAME_LENGTH);
			RFM_WeaponIdx[clientIdx] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RFM_STRING, "index");
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RFM_STRING, "attributes", RFM_WeaponArgs[clientIdx], MAX_WEAPON_ARG_LENGTH);
			RFM_WeaponVisibility[clientIdx] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RFM_STRING, "visibility");
		...
		if (FF2_HasAbility(bossIdx, PLUGIN_NAME, RSW_STRING))
		{
			// INFINITY PISTOL
			PluginActiveThisRound = true;
			...
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RSW_STRING, "classname", weaponName, sizeof(weaponName));
			new weaponIdx = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RSW_STRING, "index");
			new String:weaponArgs[MAX_WEAPON_ARG_LENGTH];
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RSW_STRING, "attributes", weaponArgs, sizeof(weaponArgs));
			new weaponVisibility = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RSW_STRING, "visibility");
			...
			RIP_AwardAmmoCount[clientIdx] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RSW_STRING, "set ammo");
			RIP_IsPrimary[clientIdx] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RSW_STRING, "use secondary") == 1;

			// MELEE WEAPON
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RSW_STRING, "melee classname", RFM_WeaponName[clientIdx], MAX_WEAPON_NAME_LENGTH);
			RFM_WeaponIdx[clientIdx] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RSW_STRING, "melee index");
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RSW_STRING, "melee attributes", RFM_WeaponArgs[clientIdx], MAX_WEAPON_ARG_LENGTH);
			RFM_WeaponVisibility[clientIdx] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RSW_STRING, "melee visibility");
...
void Ability_Init(int clientIdx)
{
	...
	// ROTT weapons
	RW_CanUse[clientIdx] = FF2_HasAbility(bossIdx, PLUGIN_NAME, RW_STRING);
	if (RW_CanUse[clientIdx])
	{
		...
		RW_WeaponCount[clientIdx] = min(FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RW_STRING, "count"), RW_MAX_WEAPONS);
		RW_WeaponVisibility[clientIdx] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RW_STRING, "visibility");
		RW_HomingInterval = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RW_STRING, "homing calculate time");

		new String:chancesStr[RW_MAX_WEAPONS * 3];
		FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RW_STRING, "odds", chancesStr, 30);
		...
		new Float:maxAngleLockOn = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RW_STRING, "lock-on angle");
		new Float:maxAngleHome = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RW_STRING, "max angle");
		...
		FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RW_STRING, "god mode sound path", godModeSounds, sizeof(godModeSounds));
		...
		RW_RJIntensityFactor = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RW_STRING, "intensity modifier");
		...
		for (new i = 0; i < RW_WeaponCount[clientIdx]; i++)
		{
			...
			RWI_Type[i] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, actualRWIString, "type");
			RWI_Duration[i] = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, actualRWIString, "duration");
			RWI_AdditionalProjectiles[i] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, actualRWIString, "additional projectile count");
			RWI_HomingDegreesPerSecond[i] = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, actualRWIString, "max degrees");
			RWI_ObsessiveHoming[i] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, actualRWIString, "re-home") == 1;
			RWI_NumAdditionalExplosions[i] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, actualRWIString, "additional explosions");
			RWI_ExplosionInterval[i] = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, actualRWIString, "explosion delay");
			RWI_RandomDeviationPerSecond[i] = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, actualRWIString, "random degrees");
			RWI_ModelOverrideIdx[i] = ReadModelToInt(bossIdx, actualRWIString, "projectile model override");
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, actualRWIString, "projectile particle override", RWI_ParticleOverride[i], MAX_EFFECT_NAME_LENGTH);
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, actualRWIString, "got weapon text", RW_Messages[i], RW_MAX_MESSAGE_LENGTH);
			...
			ReadSound(bossIdx, actualRWIString, "local rage sound path", rageSound);

			// overrides for lock on and homing angle
			RWI_LockOnAngle[i] = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, actualRWIString, "lock-on angle override");
			if (RWI_LockOnAngle[i] <= 0.0)
				RWI_LockOnAngle[i] = maxAngleLockOn;
			RWI_HomeAngle[i] = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, actualRWIString, "home angle override");
		...
		// FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RW_STRING, 19, RW_Messages[RW_RJ_FAIL], RW_MAX_MESSAGE_LENGTH);
	}

	// ROTT props
	RP_CanUse[clientIdx] = FF2_HasAbility(bossIdx, PLUGIN_NAME, RP_STRING);
	if (RP_CanUse[clientIdx])
	{
		...
		{
			index = i/2;
			Format(temp, sizeof(temp), "use %s", g_strPropName[index]);
			RP_CanDeployProp[clientIdx][index] = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RP_STRING, temp) == 1;

			Format(temp, sizeof(temp), "%s cost", g_strPropName[index]);
			RP_PropRageCost[clientIdx][index] = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RP_STRING, temp);
			if (PRINT_DEBUG_SPAM)
				PrintToServer("[sarysapub1] Prop %d (actually %d): %d / %f", i, (i/2), RP_CanDeployProp[clientIdx][i/2], RP_PropRageCost[clientIdx][i/2]);

			if (RP_CanDeployProp[clientIdx][index] && RP_CurrentlySelectedProp[clientIdx] == -1)
				RP_CurrentlySelectedProp[clientIdx] = index;
		}
		...
		// jump pad info
		if ((RP_CanDeployProp[clientIdx][PROP_JUMP_PAD] || RP_CanDeployProp[clientIdx][PROP_ANGLE_PAD]) && FF2_HasAbility(bossIdx, PLUGIN_NAME, RPJP_STRING))
		{
			RPJP_JumpPadIntensity = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RPJP_STRING, "jump pad intensity");
			RPJP_JumpPadHealth = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RPJP_STRING, "jump pad health");
			ReadModel(bossIdx, RPJP_STRING, "jump pad model", RPJP_JumpPadModel);
			ReadHull(bossIdx, RPJP_STRING, "jump pad hull", RPJP_JumpPadCollision);
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RPJP_STRING, "jump pad name", RP_PropName[PROP_JUMP_PAD], MAX_PROP_NAME_LENGTH);

			ReadSound(bossIdx, RPJP_STRING, "jump pad sound path", RPJP_JumpPadSound);

			RPJP_AnglePadIntensity = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RPJP_STRING, "angle jump pad intensity");
			RPJP_AnglePadHealth = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RPJP_STRING, "angle jump pad health");
			ReadModel(bossIdx, RPJP_STRING, "angle jump pad model", RPJP_AnglePadModel);
			ReadHull(bossIdx, RPJP_STRING, "angle jump pad hull", RPJP_AnglePadCollision);
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RPJP_STRING, "angle jump pad name", RP_PropName[PROP_ANGLE_PAD], MAX_PROP_NAME_LENGTH);
			RPJP_AnglePadDampeningFactor = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RPJP_STRING, "angle jump pad dampening factor");
		...
		if (RP_CanDeployProp[clientIdx][PROP_SLICER] && FF2_HasAbility(bossIdx, PLUGIN_NAME, RPSP_STRING))
		{
			// SLICER
			RPS_DelayBetweenChecks = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RPSP_STRING, "slicer damage time");
			RPS_DamagePerCheck = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RPSP_STRING, "slicer damage");
			RPS_NegatePushForce = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RPSP_STRING, "slicer push force") == 1;
			ReadModel(bossIdx, RPSP_STRING, "slicer model", RPS_SlicerModel);
			ReadHull(bossIdx, RPSP_STRING, "slicer hull", RPS_SlicerCollision);
			RPS_DelayBeforeDamage = FF2_GetAbilityArgumentFloat(bossIdx, PLUGIN_NAME, RPSP_STRING, "slicer delay damage");
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RPSP_STRING, "slicer name", RP_PropName[PROP_SLICER], MAX_PROP_NAME_LENGTH);
			...
			// PLATFORM
			RPP_PlatformHealth = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, RPSP_STRING, "platform health");
			ReadModel(bossIdx, RPSP_STRING, "platform model", RPP_PlatformModel);
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, RPSP_STRING, "platform name", RP_PropName[PROP_PLATFORM], MAX_PROP_NAME_LENGTH);
...
public void FF2_OnAbility(int bossIdx, const char[] plugin_name, const char[] ability_name, int slot, int status)
{
	if (strcmp(plugin_name, PLUGIN_NAME) != 0)
		return;
...
public Rage_ROTTWeapons(const String:ability_name[], bossIdx, forceWeapon)
{
	...
	new clipSize = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, actualRWIString, "clip");
	new String:weaponName[MAX_WEAPON_NAME_LENGTH];
	FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, actualRWIString, "classname", weaponName, MAX_WEAPON_NAME_LENGTH);
	...
	if (strlen(weaponName) > 3)
	{
		...
		if (shouldAddWeapon)
		{
			new weaponNum = FF2_GetAbilityArgument(bossIdx, PLUGIN_NAME, actualRWIString, "index");
			new String:weaponArgs[MAX_WEAPON_ARG_LENGTH];
			FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, actualRWIString, "attributes", weaponArgs, MAX_WEAPON_ARG_LENGTH);
	...
	FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, actualRWIString, "local rage sound path", rageSound, MAX_SOUND_FILE_LENGTH);
...
public DuplicateRocket(clientIdx, baseRocket, Float:speed, Float:spawnAngles[3], Float:zOffset)
{
	...
	new damageOffset = FindSendPropInfo(classname, "m_iDeflected") + 4; // credit to voogru
	...
	new testOffset = FindSendPropInfo(classname, "m_bCritical") - 4;
...
public MonitorRocket(clientIdx, rocket, Float:velocity)
{
	...
	if (!IsEmptyString(RWI_ParticleOverride[spec]))
	{
		...
		if (IsValidEntity(particle))
			CreateTimer(10.0, RemoveEntity_Timer, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE); // sanity timer
	}
	...
	new damageOffset = FindSendPropInfo("CTFProjectile_Rocket", "m_iDeflected") + 4; // credit to voogru
...
public DestroyProp(propIdx, bool:reorder)
{
	...
	if (IsValidEntity(prop))
		RemoveEntity_Timer(INVALID_HANDLE, prop);
...
stock ParticleEffectAt(Float:position[3], String:effectName[], Float:duration = 0.1)
{
	...
	if (particle != -1)
	{
		...
		if (duration > 0.0)
			CreateTimer(duration, RemoveEntity_Timer, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
	return particle;
}
...
public Action:RemoveEntity_Timer(Handle:timer, any:entid)
{
...
stock ReadHull(bossIdx, const String:ability_name[], const String:arg[], Float:hull[2][3])
{
	static String:hullStr[MAX_HULL_STRING_LENGTH];
	FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, ability_name, arg, hullStr, MAX_HULL_STRING_LENGTH);
	ParseHull(hullStr, hull);
}

stock ReadSound(bossIdx, const String:ability_name[], const String:arg[], String:soundFile[MAX_SOUND_FILE_LENGTH])
{
	FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, ability_name, arg, soundFile, MAX_SOUND_FILE_LENGTH);
	if (strlen(soundFile) > 3)
		PrecacheSound(soundFile);
}

stock ReadModel(bossIdx, const String:ability_name[], const String:arg[], String:modelFile[MAX_MODEL_FILE_LENGTH])
{
	FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, ability_name, arg, modelFile, MAX_MODEL_FILE_LENGTH);
	if (strlen(modelFile) > 3)
		PrecacheModel(modelFile);
}

stock ReadModelToInt(bossIdx, const String:ability_name[], const String:arg[])
{
...
stock ReadCenterText(bossIdx, const String:ability_name[], const String:arg[], String:centerText[MAX_CENTER_TEXT_LENGTH])
{
	FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, ability_name, arg, centerText, MAX_CENTER_TEXT_LENGTH);
...
stock ReadHexOrDecString(bossIdx, const String:ability_name[], const char[] arg)
{
	static String:hexOrDecString[HEX_OR_DEC_STRING_LENGTH];
	FF2_GetAbilityArgumentString(bossIdx, PLUGIN_NAME, ability_name, arg, hexOrDecString, HEX_OR_DEC_STRING_LENGTH);
...
stock Float:RandomNegative(Float:i_n)
{
	return i_n * (GetRandomInt(0, 1) == 1 ? 1.0 : -1.0);
}
